...
const createNote = async (req, res) => {
    try {
        const { title, description, category, uploadsRatingAndReview, tags } = req.body;
...
const getRecommendedNotes = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Recommended notes fetched successfully", notes: recommendedNotes });
    } catch (error) {
        res.status(500).json({ message: "Error fetching recommended notes", error: error.message });
    }
};
...
const getNoteDetails = async (req, res) => {
    try {
        const { noteId } = req.params;
        const note = await Note.findById(noteId).populate("user category uploadsRatingAndReview tags");
        if (!note) {
            return res.status(404).json({ message: "Note not found" });
        }
        ...
        res.status(500).json({ message: "Error fetching note details", error: error.message });
    }
};
...
const getFavouriteNotes = async (req, res) => {
    try {
        ...
        const favouriteNotes = await Note.find({ user: req.user._id, favourite: true });
        res.status(200).json({ message: "Favourite notes fetched successfully", notes: favouriteNotes });
    } catch (error) {
        res.status(500).json({ message: "Error fetching favourite notes", error: error.message });
    }
};
...
const getBookmarkedNotes = async (req, res) => {
    try {
        ...
        const bookmarkedNotes = await Note.find({ user: req.user._id, bookmarked: true });
        res.status(200).json({ message: "Bookmarked notes fetched successfully", notes: bookmarkedNotes });
    } catch (error) {
        res.status(500).json({ message: "Error fetching bookmarked notes", error: error.message });
    }
};
